...
> Кроме *Docker Compose* также существует инструмент *Docker Swarm*, который предлагает подобную функциональность, но с разницей, что *Docker Compose* запускает и управляет контейнерами на одном хосте, тогда как *Docker Swarm* управляет контейнерами на различных хостах (выполняет *оркестрацию*).
